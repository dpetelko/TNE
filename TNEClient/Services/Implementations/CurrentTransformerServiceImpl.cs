using Serilog;
using System;
using System.Collections.Generic;
using System.Net.Http;
using System.Threading.Tasks;
using TNEClient.Data;
using TNEClient.Dtos;

namespace TNEClient.Services.Implementations
{
    public class CurrentTransformerServiceImpl : ICurrentTransformerService
    {
        private readonly ICurrentTransformerRepository _repo;

        public CurrentTransformerServiceImpl(ICurrentTransformerRepository repo) { _repo = repo; }

        public async Task<CurrentTransformerDto> CreateAsync(CurrentTransformerDto dto)
        {
            return await _repo.CreateAsync(dto);
        }

        public async Task<List<CurrentTransformerDto>> GetAllAsync()
        {
            return await _repo.GetAllAsync();
        }

        public async Task<List<CurrentTransformerDto>> GetAllByStatusAsync(Status status)
        {
            return await _repo.GetAllByStatusAsync(status);
        }

        public async Task<CurrentTransformerDto> GetAsync(Guid id)
        {
            return await _repo.GetAsync(id);
        }

        public async Task<CurrentTransformerDto> UpdateAsync(CurrentTransformerDto dto)
        {
            return await _repo.UpdateAsync(dto);
        }

        public async Task<bool> SetStatus(Guid id, Status status)
        {
            return await _repo.SetStatus(id, status);
        }
    }
}
